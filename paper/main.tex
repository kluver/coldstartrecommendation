%TODO: diversity config matches item-item config.
%TODO: review results
...
\section{Introduction}
  ...
  Every potential user of the system must at some point be a new user.
  If the system cannot provide a good user experience, then the user is likely to leave and not return.
...
% one important trait is trust.
  ...
  Past work on new user experience in recommender systems has tended to focus on the algorithmic cold start problem.
  The cold start problem refers to the problem that no algorithm can make useful recommendations with very limited amounts of information.
  While this applies to both new users and new items we will only discuss research on the new user cold start problem.
  Most solutions to the algorithmic cold start problem require some source of extra information about new users.
  ...
  Generally, this work strives to strike a balance between the amount of effort the user must put in before getting enough ratings, and the usefulness of each rating to the user.
  ...
  They found that users who were in control of which items to rate during a new user survey rated fewer movies, but that the profiles they created were more useful to the system in predicting their future behavior.
  Interestingly, while these users took more time to create their profiles, they reported that the sign up was short at the same rate as the system driven condition.
  ...
  Novel interfaces may elicit information other than ratings such as item comparisons \cite{Loepp}.
  Furthermore, very little work has carefully considered how best to use the information we receive.
  ...
  We propose to carefully compare the performance of standard algorithms at recommending for new users.
  ...
  Our approach is related to temporal evaluations of recommender systems \cite{KorenTemporal, LathiaTemporal, burke2010evaluating}.
  Temporal evaluations tend to evaluate recommender systems as a function of time, seeing how they performed at different points in their lifetime.
  This approach is well suited for understanding the effect of changes to the system, or user base of a recommender.
  Unfortunately, this approach is less well suited towards comparing algorithms, as it necessarily co-mingles the effect of a recommender on user navigation with the quality of the recommender making it hard to consider algorithms that were not deployed with the technique.
  Regardless, we consider this technique an inspiration for our evaluation.
  %TODO: I kind of want to cut this paragraph
  %TODO: Our work sits at the intersection of two important traditions of recommender systems research: new user experience, and offline algorithm comparison.
