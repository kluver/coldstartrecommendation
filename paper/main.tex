...
%  One of the most traditional ways to measure the behavior of a recommendation system is to measure its performance on the rating task.
%  This is normally done with metrics such as RMSE or MAE.
%  The accuracy of the algorithm may be important at cold start as users my judge the recommender on how well it predicts items they have not yet rated.
%  This early judgment may be very important to the user.

%  Recently, work in the recommender system community has been shifting away from prediction accuracy to start looking at how well the system ranks and recommends items.
%  This change was, in part prompted by work criticizing the practice of measuring only the RMSE of recommender systems \cite{TODO:rmse considered harmful...}
%  The quality of the ranking provided by the recommender has normally been measured with information retrial metrics such as nDCG.
%  The quality of the recommendations has normally been measured by looking at which sort of items end up being recommended with metrics such as precision@N, recall@N, fallout@N etc.
%  We will also look at some simpler metrics along with these such as simply evaluating the ratings and predictions that end up in top 20 recommendations.
